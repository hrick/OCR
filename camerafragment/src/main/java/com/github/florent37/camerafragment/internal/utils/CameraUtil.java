...
public class CameraUtil {
    ...
    public static boolean isCorrectRatio(Size size) {
        float photoRatio = (float) size.getHeight() / (float) size.getWidth();
        float ratioPortrait = (float) 3 / (float) 4;
        float ratioLand = (float) 4 / (float) 3;
        return photoRatio == ratioPortrait || photoRatio == ratioLand;
    }
