...
import static com.github.florent37.camerafragment.internal.enums.Camera.ORIGINAL_CAMERA2_IMAGE_WIDTH;
import static com.github.florent37.camerafragment.internal.enums.Camera.ORIGINAL_CAMERA2_IMAGE_HEIGHT;
    ...
    private void onPictureStart(final CameraFragmentResultListener callback) {
        if (cameraPhotoListener != null) {
            uiHandler.post(new Runnable() {
                @Override
                public void run() {
                    cameraPhotoListener.onPhotoStart(callback);
                }
            });
        }
    }
...
//            photoSize = new Size();
//            photoSize.setHeight(ORIGINAL_CAMERA2_IMAGE_HEIGHT);
//            photoSize.setWidth(ORIGINAL_CAMERA2_IMAGE_WIDTH);
            photoSize = CameraHelper.findClosetPictureSizeCamera2(Size.fromArray2(map.getOutputSizes(ImageFormat.JPEG)), ORIGINAL_IMAGE_WIDTH, ORIGINAL_IMAGE_HEIGHT);
...
//                    previewSize = CameraHelper.getOptimalPreviewSize(Size.fromArray2(map.getOutputSizes(SurfaceTexture.class)), photoSize.getWidth(), photoSize.getHeight());
                    previewSize = CameraHelper.findClosestPreviewSizeCamera2(Size.fromArray2(map.getOutputSizes(SurfaceTexture.class)), new Point(photoSize.getWidth(), photoSize.getHeight()));
...
//                    previewSize = CameraHelper.getOptimalPreviewSize(Size.fromArray2(map.getOutputSizes(SurfaceTexture.class)), windowSize.getWidth(), windowSize.getHeight());
                    previewSize = CameraHelper.findClosestPreviewSizeCamera2(Size.fromArray2(map.getOutputSizes(SurfaceTexture.class)), new Point(windowSize.getWidth(), windowSize.getHeight()));
    ...
    private void captureStillPicture() {
        try {
            ...
            final CaptureRequest.Builder captureBuilder = previewRequestBuilder;
            ...
            CameraCaptureSession.CaptureCallback CaptureCallback = new CameraCaptureSession.CaptureCallback() {
                ...
                public void onCaptureCompleted(@NonNull CameraCaptureSession session,
                                               @NonNull CaptureRequest request,
                                               @NonNull TotalCaptureResult result) {
                    if (flashMode == Configuration.FLASH_MODE_ON) {
                        setFlashMode(Configuration.FLASH_MODE_OFF);
                        flashMode = Configuration.FLASH_MODE_OFF;
                    }
        ...
        try {
            ...
            if ((float) level <= maxzoom) {
                ...
                if (previewRequestBuilder != null) {
                    ...
                    int cropW = difW / 100 * level;
                    int cropH = difH / 100 * level;
                    cropW -= cropW & 3;
                    cropH -= cropH & 3;
    ...
    private void setFlashModeAndBuildPreviewRequest(@Configuration.FlashMode int flashMode) {
        try {
            ...
            boolean supportFlash = backCameraCharacteristics.get(CameraCharacteristics.FLASH_INFO_AVAILABLE);
            if (supportFlash)
                switch (flashMode) {
    ...
    public void onImageAvailable(final ImageReader imageReader) {
        ...
        backgroundHandler.post(new ImageSaver(imageReader.acquireNextImage(), outputFile, new ImageSaver.ImageSaverCallback() {
            ...
            public void onSuccessFinish(final byte[] bytes) {
                ...
                unlockFocus();
//                // TODO SE COMEÇAR A TER RECLAMAÇÃO MEXER AQUI
//                if (imageReader.acquireNextImage() != null){
//                    Log.e("CAMERA2", "imageReader.acquireNextImage().close()");
